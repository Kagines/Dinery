  ...
  export default {
    ...
    methods: {
      async fetchRestaurant() {
        ...
        try {
          const response = await axios.get(`http://localhost:3000/restaurant/${restaurantId}`);
      ...
      async fetchReviews() {
        const restaurantId = this.$route.params.id;
        try {
          const response = await axios.get(`http://localhost:3000/reviews?restaurantId=${restaurantId}`);
      ...
      async submitReview() {
        ...
        try {
          ...
          if (response.status === 201) {
            ...
            this.newReview = { title: '', comments: '', rating: null, name: 'anonymous', date: new Date().toISOString().split('T')[0] };
          } else {
